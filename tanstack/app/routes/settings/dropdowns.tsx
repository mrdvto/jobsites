...
import { useLookups, useNoteTags, useUpdateLookups, useUpdateNoteTags } from '@/hooks/useLookups';
...
function ManageDropdowns() {
  ...
  const updateNoteTags = useUpdateNoteTags();
  ...
  const [dropdowns, setDropdowns] = useState<Record<DropdownType, DropdownOption[]>>(() => {
    ...
    return {
      ...
      primaryStage: (lookups?.primaryStages || initialDropdowns.primaryStage).map(s => ({ id: s.id, label: s.label, displayOrder: s.displayOrder })),
      primaryProjectType: (lookups?.primaryProjectTypes || initialDropdowns.primaryProjectType).map(t => ({ id: t.id, label: t.label, displayOrder: t.displayOrder })),
      ownershipType: (lookups?.ownershipTypes || initialDropdowns.ownershipType).map(o => ({ id: o.id, label: o.label, displayOrder: o.displayOrder })),
    };
  });
  const [isEditing, setIsEditing] = useState(false);
  const [editedValues, setEditedValues] = useState<DropdownOption[]>([]);
  const [deleteConfirm, setDeleteConfirm] = useState<{ open: boolean; item: DropdownOption | null }>({ open: false, item: null });
  const [newItem, setNewItem] = useState<{ label: string; displayOrder: string; color: string }>({ label: '', displayOrder: '', color: 'emerald' });
  ...
  const dropdownOptions: { key: DropdownType; label: string }[] = [
    ...
    { key: 'noteTags', label: 'Note Tags' },
    { key: 'primaryStage', label: 'Primary Stage' },
    { key: 'primaryProjectType', label: 'Primary Project Type' },
    { key: 'ownershipType', label: 'Ownership Type' },
  ];

  const sortByDisplayOrder = (items: DropdownOption[]) => [...items].sort((a, b) => a.displayOrder - b.displayOrder);
  ...
  const syncToServer = (type: DropdownType, values: DropdownOption[]) => {
    if (type === 'projectStatus') {
      const colorMap: Record<string, string> = {};
      values.forEach(item => { if (item.color) colorMap[item.id] = item.color; });
      updateAllStatusColors(colorMap);
    }
    if (type === 'noteTags') {
      updateNoteTags.mutate({ tags: values.map(item => ({ id: item.id, label: item.label, displayOrder: item.displayOrder, color: item.color || 'slate' })) });
    }
    if (type === 'primaryStage') {
      updateLookups.mutate({ primaryStages: values.map(i => ({ id: i.id, label: i.label, displayOrder: i.displayOrder })) });
    }
    if (type === 'primaryProjectType') {
      updateLookups.mutate({ primaryProjectTypes: values.map(i => ({ id: i.id, label: i.label, displayOrder: i.displayOrder })) });
    }
    if (type === 'ownershipType') {
      updateLookups.mutate({ ownershipTypes: values.map(i => ({ id: i.id, label: i.label, displayOrder: i.displayOrder })) });
    }
  };

  const handleSaveEdit = () => {
    if (selectedDropdown) {
      setDropdowns(prev => ({ ...prev, [selectedDropdown]: editedValues }));
      syncToServer(selectedDropdown, editedValues);
      setIsEditing(false);
      toast({ title: 'Changes saved', description: 'Dropdown values have been updated.' });
    }
  };
  ...
  const handleDeleteClick = (item: DropdownOption) => setDeleteConfirm({ open: true, item });
  ...
  const handleConfirmDelete = () => {
    if (deleteConfirm.item && selectedDropdown) {
      ...
      if (isEditing) {
        ...
        setDropdowns(prev => ({ ...prev, [selectedDropdown]: prev[selectedDropdown].filter(v => v.id !== deleteConfirm.item!.id) }));
      }
      toast({ title: 'Item deleted', description: `"${deleteConfirm.item.label}" has been removed.` });
    }
    setDeleteConfirm({ open: false, item: null });
  };

  const generateId = (label: string) => label.toUpperCase().replace(/\s+/g, '-').replace(/[^A-Z0-9-]/g, '');

  const handleAddItem = () => {
    if (!newItem.label) {
      toast({ title: 'Missing fields', description: 'Label is required.', variant: 'destructive' });
      return;
    }
    if (selectedDropdown) {
      ...
      if (currentValues.some(v => v.id === generatedId)) {
        toast({ title: 'Duplicate entry', description: 'An item with this label already exists.', variant: 'destructive' });
        return;
      }
      ...
      if (isEditing) {
        ...
        const updatedValues = [...dropdowns[selectedDropdown], newOption];
        setDropdowns(prev => ({ ...prev, [selectedDropdown]: updatedValues }));
        syncToServer(selectedDropdown, updatedValues);
      }
      setNewItem({ label: '', displayOrder: '', color: 'emerald' });
      setShowAddForm(false);
      toast({ title: 'Item added', description: `"${newItem.label}" has been added.` });
    }
  };
      ...
      <main className="container mx-auto px-6 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="md:col-span-1">
            <CardHeader><CardTitle className="text-lg">Dropdowns</CardTitle></CardHeader>
            <CardContent className="p-2">
              <div className="space-y-1">
                {dropdownOptions.map(option => (
                  <Button key={option.key} variant={selectedDropdown === option.key ? 'default' : 'ghost'} className="w-full justify-start" onClick={() => handleSelectDropdown(option.key)}>
                    {option.label}
                  </Button>
                ))}
              </div>
          ...
          <Card className="md:col-span-3">
              ...
              {selectedDropdown && (
                <div className="flex gap-2">
                  {isEditing ? (
                    ...
                    <>
                      <Button variant="outline" size="sm" onClick={() => setShowAddForm(true)}><Plus className="h-4 w-4 mr-1" />Add</Button>
                      <Button variant="outline" size="sm" onClick={handleStartEdit}><Pencil className="h-4 w-4 mr-1" />Edit</Button>
                    </>
                  )}
                </div>
              )}
            ...
            <CardContent>
              {selectedDropdown ? (
                  ...
                  {showAddForm && !isEditing && (
                    <div className="mb-4 p-4 border rounded-md bg-muted/50">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <Input placeholder="Label" value={newItem.label} onChange={e => setNewItem(prev => ({ ...prev, label: e.target.value }))} />
                        <Input placeholder="Display Order (optional)" type="number" value={newItem.displayOrder} onChange={e => setNewItem(prev => ({ ...prev, displayOrder: e.target.value }))} />
                      </div>
                      {(selectedDropdown === 'projectStatus' || selectedDropdown === 'noteTags') && (
                        <div className="mt-3">
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {STATUS_COLORS.map(c => (
                              <button key={c.id} type="button" onClick={() => setNewItem(prev => ({ ...prev, color: c.id }))}
                                className={`w-8 h-8 rounded-full ${c.bg} border-2 ${newItem.color === c.id ? 'border-primary ring-2 ring-primary/30' : 'border-transparent'}`} title={c.label} />
                            ))}
                          </div>
                        </div>
                      )}
                      <div className="flex gap-2 mt-3">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => { setShowAddForm(false); setNewItem({ label: '', displayOrder: '', color: 'emerald' }); }}>Cancel</Button>
                      </div>
                    </div>
                  )}
                  <Table>
                              ...
                              <TableCell>
                                {isEditing ? (
                                  <div className="flex gap-1 flex-wrap">
                                    {STATUS_COLORS.map(c => (
                                      <button key={c.id} type="button" onClick={() => handleEditValue(editIndex, 'color', c.id)}
                                        className={`w-6 h-6 rounded-full ${c.bg} border-2 ${editedValues[editIndex]?.color === c.id ? 'border-primary ring-2 ring-primary/30' : 'border-transparent'}`} title={c.label} />
                                    ))}
                                  </div>
                                ) : (
                                  <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${colorConfig.bg} ${colorConfig.text}`}>{colorConfig.label}</span>
                                )}
                              </TableCell>
                            )}
                            <TableCell>
                ...
                <div className="text-center py-8 text-muted-foreground">Select a dropdown from the list to view and edit its values</div>
              )}
            </CardContent>
          </Card>
        </div>
      ...
      <AlertDialog open={deleteConfirm.open} onOpenChange={open => setDeleteConfirm({ open, item: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to delete "{deleteConfirm.item?.label}"? This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
