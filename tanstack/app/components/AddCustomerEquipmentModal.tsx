...
import { useEquipmentProjectAssignment } from '@/hooks/useProjects';
...
export const AddCustomerEquipmentModal = ({ open, onOpenChange, onSave, projectId, projectCompanies, existingEquipmentIds }: AddCustomerEquipmentModalProps) => {
  ...
  const { data: companyEquipment = [] } = useCompanyEquipment(companyId);
  const { data: equipmentAssignment } = useEquipmentProjectAssignment(selectedEquipmentId ?? 0, projectId);

  const availableEquipment = useMemo(
    () => companyEquipment.filter(eq => !existingEquipmentIds.includes(eq.id)),
    [companyEquipment, existingEquipmentIds]
  );
  ...
  const handleSubmit = () => {
    ...
    if (equipmentAssignment) {
      setConflictInfo({ projectName: equipmentAssignment.projectName });
