...
import { ArrowLeft, MapPin, User, Phone, Mail, Building2, Plus, Link as LinkIcon, X, Pencil, Calendar, Wrench, Search, ChevronRight, ArrowUpDown, ArrowUp, ArrowDown, History, ExternalLink, CornerDownRight, Link2, DollarSign, FileText, ClipboardList, Filter } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { Activity, ProjectCompany, CustomerEquipment } from '@/types';

// Hooks
import { useProject, useRemoveProjectCompany, useUpdateProject, useDeleteActivity, useAddNote, useUpdateNote, useDeleteNote, useAddCustomerEquipment, useDeleteCustomerEquipment, useAddProjectCompany, useAllKnownCompanies } from '@/hooks/useProjects';
...
import { getSalesRepName as getSalesRepNameHelper, getSalesRepNames as getSalesRepNamesHelper, getUserName as getUserNameHelper, getUserNames as getUserNamesHelper, getStageName as getStageNameHelper, getStage as getStageHelper, getLookupLabel as getLookupLabelHelper } from '@/lib/lookupHelpers';
...
function ProjectDetailPage() {
  ...
  const getSalesRepNames = (ids: number[]) => getSalesRepNamesHelper(allSalesReps, ids);
  ...
  const [showUom, setShowUom] = useState(() => typeof window !== 'undefined' ? localStorage.getItem('showEquipmentUom') === 'true' : false);
  ...
  const handleSaveNewEquipment = (equipment: CustomerEquipment) => {
    ...
    addCustomerEquipmentMutation.mutate({ projectId: project.id, equipmentId: equipment.id, equipmentLabel: `${equipment.make} ${equipment.model}`, userId: currentUserId });
  ...
  const handleDeleteEquipment = () => {
    if (equipmentToDelete !== null) {
      ...
      deleteCustomerEquipmentMutation.mutate({ projectId: project.id, equipmentId: equipmentToDelete, equipmentLabel: eq ? `${eq.make} ${eq.model}` : undefined, userId: currentUserId });
  ...
  const handleSaveEquipment = (equipmentId: number) => {
    ...
    addCustomerEquipmentMutation.mutate({ projectId: project.id, equipmentId, equipmentLabel: eq ? `${eq.make} ${eq.model}` : undefined, userId: currentUserId });
  ...
  const uniqueStages = [...new Map(project.associatedOpportunities.map((o: any) => [o.stageId, getStageName(o.stageId)] as [number, string])).entries()].sort((a, b) => (a[1] as string).localeCompare(b[1] as string));
  const uniqueTypes = [...new Set<string>(project.associatedOpportunities.map((o: any) => o.type as string))].sort();
  const uniqueDivisionsArr = project.associatedOpportunities.map((o: any) => {
    const full = opportunities.find((f: any) => f.id === o.id);
    return full?.divisionId || '';
  }).filter((d: string) => d !== '') as string[];
  const uniqueDivisions = [...new Set<string>(uniqueDivisionsArr)].sort();

  const uniqueOppSalesRepsArr = project.associatedOpportunities.map((o: any) => {
    const full = opportunities.find((f: any) => f.id === o.id);
    return full ? getSalesRepName(full.salesRepId) : '';
  }).filter((r: string) => r !== '') as string[];
  const uniqueOppSalesReps = [...new Set<string>(uniqueOppSalesRepsArr)].sort();

  const uniqueOppCompaniesArr = project.associatedOpportunities.map((o: any) => {
    const full = opportunities.find((f: any) => f.id === o.id);
    return full?.customerName || '';
  }).filter((c: string) => c !== '') as string[];
  const uniqueOppCompanies = [...new Set<string>(uniqueOppCompaniesArr)].sort();
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-6 py-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <Button variant="outline" size="sm" onClick={() => navigate({ to: "/project-changelog/$id", params: { id: String(project.id) } })} className="ml-2">
      ...
      <main className="container mx-auto px-6 py-8">
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {(hasAddress || hasCoordinates) &&
                      <div className="flex rounded-md border border-input overflow-hidden">
                        <Button type="button" variant={locationViewType === "address" ? "default" : "ghost"} size="sm" className="rounded-none h-7 text-xs" onClick={() => setLocationViewType("address")} disabled={!hasAddress}>Address</Button>
                        <Button type="button" variant={locationViewType === "coordinates" ? "default" : "ghost"} size="sm" className="rounded-none h-7 text-xs" onClick={() => setLocationViewType("coordinates")} disabled={!hasCoordinates}>Coordinates</Button>
                      </div>
                    }
                  </div>
            ...
            <>
              <div className="flex gap-3 mb-4 flex-wrap">
                ...
                <div className="flex items-center space-x-2 ml-auto">
                  <Switch id="oppShowOpenOnly" checked={oppShowOpenOnly} onCheckedChange={setOppShowOpenOnly} />
                  <Label htmlFor="oppShowOpenOnly" className="text-sm font-normal cursor-pointer">Show Open Only</Label>
                </div>
              </div>
                    ...
                    {sortedOpportunities.map((opp) => {
                      ...
                      return (
                        <TableRow key={opp.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleOpportunityClick(opp.id)}>
                          ...
                          <TableCell className="text-right font-medium">${opp.revenue.toLocaleString("en-US", { maximumFractionDigits: 0 })}</TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button size="sm" onClick={() => setShowAssociateCompanyModal(true)}>
                <LinkIcon className="h-4 w-4 mr-2" />
                Associate Existing
              </Button>
            </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setShowActFilterModal(true)}>
                      ...
                      <TableRow key={activity.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            ...
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => initiateDeleteActivity(activity.id)}>
                              <X className="h-4 w-4" />
                            </Button>
                          </div>
                        </TableCell>
        ...
        <Card className="p-6">
          ...
          {!project.customerEquipment || project.customerEquipment.length === 0 ?
            ...
            (() => {
              ...
              const grouped: Record<string, { companyName: string; items: CustomerEquipment[] }> = filteredEquipment.reduce<Record<string, { companyName: string; items: CustomerEquipment[]; }>>((acc, eq) => {
                if (!acc[eq.companyId]) {
                  acc[eq.companyId] = {
              ...
              const groupEntries = Object.entries(grouped) as [string, { companyName: string; items: CustomerEquipment[] }][];
