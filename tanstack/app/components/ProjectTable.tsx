...
import { useProjects, useAllKnownCompanies } from '@/hooks/useProjects';
import { useOpportunities } from '@/hooks/useOpportunities';
import { useOpportunityStages, useLookups, useUsers } from '@/hooks/useLookups';
...
import { calculateProjectWonRevenue, calculateProjectPipelineRevenue } from '@/lib/revenue';
import { getLookupLabel as getLookupLabelHelper } from '@/lib/lookupHelpers';
...
import type { Project, LookupOption } from '@/types';
...
export const ProjectTable = () => {
  ...
  const allCompanies = allKnownCompanies || [];
  const primaryStages: LookupOption[] = lookups?.primaryStages || [];
  const primaryProjectTypes: LookupOption[] = lookups?.primaryProjectTypes || [];
  const ownershipTypes: LookupOption[] = lookups?.ownershipTypes || [];

  const filteredProjects = filterProjects(allProjects, filters, allOpportunities);
  ...
  const getCompanyById = (id: string) => {
    return allCompanies.find(c => c.companyId === id);
  };

  const getLookupLabel = (type: 'primaryStage' | 'primaryProjectType' | 'ownershipType', id: string): string => {
    const options = type === 'primaryStage' ? primaryStages : type === 'primaryProjectType' ? primaryProjectTypes : ownershipTypes;
    return getLookupLabelHelper(options, id);
  };

  const calcWonRevenue = (p: Project) => calculateProjectWonRevenue(p);
  const calcPipelineRevenue = (p: Project) => calculateProjectPipelineRevenue(p, allStages);

  const helpers: RenderHelpers = {
    getUserNames,
    getCompanyById,
    calculateProjectWonRevenue: calcWonRevenue,
    calculateProjectPipelineRevenue: calcPipelineRevenue,
  ...
  const sortedProjects = [...searchedProjects].sort((a, b) => {
    ...
    switch (sortColumn) {
      ...
      case 'status':
        ...
        break;
      case 'wonRevenue':
        comparison = calcWonRevenue(a) - calcWonRevenue(b);
        break;
      case 'pipelineRevenue':
        comparison = calcPipelineRevenue(a) - calcPipelineRevenue(b);
        break;
      case 'valuation':
            ...
            {paginatedProjects.length === 0 ? (
              ...
              paginatedProjects.map((project) => (
                <TableRow
                  ...
                  onClick={() => navigate({ to: '/project/$id', params: { id: String(project.id) } })}
                >
